...
function publishingColor(s: string) {
  ...
  if (s === "قيد المراجعة") return "bg-amber-100 text-amber-800 border-amber-200";
    ...
    <Dialog>
      ...
      <DialogContent dir="rtl" className="max-h-[88vh] overflow-y-auto sm:max-w-2xl">
        ...
        <div className="mt-2 space-y-2 text-sm">
          ...
          <DetailRow label="Funnel" value={entry.funnel.join("، ")} />
          <DetailRow label="نوع المحتوى" value={entry.typeOfContent} />
          <DetailRow label="عضوي / مدفوع" value={entry.orgPaid} />
          <DetailRow label="حالة النشر" value={entry.publishing} />
          <DetailRow label="القنوات" value={entry.channels.join("، ")} />
          <DetailRow label="Caption SA 🇸🇦" value={entry.captionSA} multiline />
          <DetailRow label="Caption EG 🇪🇬" value={entry.captionEG} multiline />
          <DetailRow
            label="Script"
            value={entry.script}
            link={entry.script?.startsWith("http") ? entry.script : undefined}
          />
          <DetailRow label="TOV" value={entry.tov} />
          ...
          <DetailRow
            ...
            link={entry.reelLink?.startsWith("http") ? entry.reelLink : undefined}
          />
          <DetailRow
            label="Storyboard"
            value={entry.storyboard}
            link={entry.storyboard?.startsWith("http") ? entry.storyboard : undefined}
          />
          <DetailRow label="Material" value={entry.material} />
          <DetailRow label="المقاس" value={entry.size} />
          ...
          <DetailRow label="وقت النشر" value={entry.publishingTime} />
          <DetailRow
            label="رابط المحتوى"
            value={entry.contentLink}
            link={entry.contentLink?.startsWith("http") ? entry.contentLink : undefined}
          />
          <DetailRow label="الكود" value={entry.code} />
          <DetailRow label="جاهز للنشر؟" value={entry.readyToPublish} />
          <DetailRow label="تمت المراجعة؟" value={entry.reviewed} />
...
export function ViewTable({ entries }: Props): ReactElement {
  ...
  return (
    <div className="space-y-4">
      {/* Toolbar — filters only, no add button */}
      <div className="flex flex-wrap items-center gap-2">
        <Input
          placeholder="بحث في الأفكار والكابشنات..."
          value={searchQ}
