...
export function EntryPageForm({ mode, entryId, slug, clientId, month, monthLabel, defaultValues }: Props): ReactElement {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (mode === "create") {
        const result = await createEntry({ month, clientId, ...commonFields });
        if (result.success) {
